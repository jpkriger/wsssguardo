...
export default function RiskOverview({ projectId }: RiskOverviewProps): ReactElement {
	...
	useEffect(() => {
		...
		async function loadRiskOverview(): Promise<void> {
			...
			try {
				...
				setRisks([]);
				setFindings([]);
				setError(loadError instanceof Error ? loadError.message : "Tabela indisponível.");
			} finally {
				if (!cancelled) {
					setLoading(false);
				}
	...
	const rows = useMemo<RiskRow[]>(() => {
		const findingMap = new Map(findings.map((finding) => [finding.id, finding.name]));

		return risks.map((risk, index) => ({
			id: risk.id,
			name: risk.name,
			riskLevelLabel: getRiskLevelConfig(risk.riskLevel),
			businessImpact: risk.consequences?.trim() || risk.description?.trim() || "Sem descrição.",
			linkedFindings: risk.findIds.length > 0
				? risk.findIds.map((findingId) => findingMap.get(findingId) ?? findingId).join(", ")
					...
					{loading ? (
						...
						rows.map((row, index) => (
							...
							>
								...
								<td className="px-4 py-4 text-slate-600">{row.businessImpact}</td>
								<td className="px-4 py-4 text-slate-600">{row.linkedFindings}</td>
...
function PriorityPill({ priority }: { priority: PriorityLabel }): ReactElement {
	...
	return (
		<span className={`inline-flex items-center gap-1 rounded-md border px-2.5 py-1 text-xs font-medium ${colorClasses}`}>
			{priority}
		</span>
	);
}
