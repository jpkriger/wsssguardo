...
import { AlertTriangle, MapPin, Database, Scale, Wrench, LayoutGrid, ArrowRight, FileText, Users, Building } from "lucide-react";
...
function buildGetRiskLevelConfig(
  ...
  const configs: RiskLevelConfig[] = sorted.map((_, i) => {
    if (i === sorted.length - 1)
      return { label: sorted[i].label as RiskLevelLabel, badgeClassName: "border-red-200 bg-red-50 text-red-600", borderColor: "border-red-200" };
    if (i === sorted.length - 2)
      return { label: sorted[i].label as RiskLevelLabel, badgeClassName: "border-orange-200 bg-orange-50 text-orange-600", borderColor: "border-orange-200" };
    if (i === sorted.length - 3)
      return { label: sorted[i].label as RiskLevelLabel, badgeClassName: "border-amber-200 bg-amber-50 text-amber-600", borderColor: "border-amber-200" };
    return { label: sorted[i].label as RiskLevelLabel, badgeClassName: "border-emerald-200 bg-emerald-50 text-emerald-700", borderColor: "border-emerald-200" };
  });
...
function getMockRegulatoryTags(risk: RiskResponse): string[] {
  if (risk.riskLevel >= 75) return ["LGPD", "ISO 27001", "Auditoria obrigatória"];
...
export default function RiskAnalysis({ projectId }: RiskAnalysisProps): ReactElement {
  const [risks, setRisks] = useState<RiskResponse[]>([]);
  const [findings, setFindings] = useState<FindingResponse[]>([]);
  const [assets, setAssets] = useState<AssetResponse[]>([]);
  const [artifacts, setArtifacts] = useState<ArtifactResponse[]>([]);
  const [riskCategories, setRiskCategories] = useState<RiskCategoryDTO[]>(DEFAULT_RISK_CATEGORIES);
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      if (!projectId) {
        setRisks([]); setFindings([]); setAssets([]); setArtifacts([]);
        setLoading(false);
        return;
      }
      setLoading(true);
      ...
      try {
        ...
        setAssets(assetRes.status === "fulfilled" ? assetRes.value.content : []);
        setArtifacts(artifactRes.status === "fulfilled" ? artifactRes.value : []);
        if (configRes.status === "fulfilled" && configRes.value.riskConfig.categories.length > 0) {
          setRiskCategories(configRes.value.riskConfig.categories);
        }
      } catch (err) {
        if (!cancelled) setError(err instanceof Error ? err.message : "Análise indisponível.");
      } finally {
  ...
  const getRiskLevelConfig = useMemo(() => buildGetRiskLevelConfig(riskCategories), [riskCategories]);
  ...
  const items = useMemo<RiskAnalysisItem[]>(() => {
    ...
    return risks.map((risk) => {
      ...
      const linkedArtifactIds = linkedFindings.flatMap((f) => f.linkedArtifactIds);
      ...
      return {
        ...
        linkedArtifactNames: getUniqueNames(linkedArtifactIds, artifactMap).slice(0, 3),
        potentialConsequences: getPotentialConsequences(risk),
      };
    });
  }, [assets, artifacts, findings, getRiskLevelConfig, risks]);

  if (!projectId)
    return <StatusBox>Selecione um projeto para visualizar a análise de riscos.</StatusBox>;
  if (loading)
    return <StatusBox>Carregando análise de riscos...</StatusBox>;
  if (error)
    return <StatusBox variant="error">{error}</StatusBox>;
  if (items.length === 0)
  ...
  return (
    <div className="my-8 space-y-4">
      <div className="space-y-2">
        <h2 className="text-2xl font-semibold text-slate-700">Análise dos Riscos</h2>
...
function RiskCard({ item }: { item: RiskAnalysisItem }): ReactElement {
  ...
  return (
    <div className={`rounded-2xl border ${item.riskLevel.borderColor} bg-white overflow-hidden`}>
      {/* Header */}
      <div className="flex items-start justify-between gap-4 px-6 py-5">
        <div className="flex items-center gap-3">
          <AlertTriangle className="shrink-0 text-slate-400 mt-0.5" size={20} strokeWidth={1.5} />
          <h3 className="text-lg font-semibold text-slate-900">{item.risk.name}</h3>
        </div>
        <Badge
          variant="outline"
      ...
      <div className="px-6 pb-6 space-y-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600 leading-relaxed">
            {item.risk.description || "Sem descrição disponível para este risco."}
          </p>
        </div>

        {/* Location + Affected Asset */}
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-1">
              <Database size={13} className="text-slate-400" strokeWidth={1.5} />
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            <SectionLabel>Evidências de suporte (ativos + auditoria)</SectionLabel>
                ...
                <ul className="space-y-1">
                  {item.linkedArtifactNames.map((name) => (
                    <li key={name} className="flex items-center gap-1.5 text-sm text-slate-600">
                      <FileText size={13} className="text-slate-400 shrink-0" strokeWidth={1.5} />
                      {name}
                    </li>
                  ))}
                </ul>
              ...
              <div className="mt-3">
                ...
                <ul className="space-y-1">
                  {item.linkedAssetNames.slice(1).map((name) => (
                    <li key={name} className="flex items-center gap-1.5 text-sm text-slate-600">
                      <Users size={13} className="text-slate-400 shrink-0" strokeWidth={1.5} />
                      {name}
                    </li>
                  ))}
                </ul>
            ...
            {item.linkedArtifactNames.length === 0 && item.linkedAssetNames.length <= 1 && (
              <p className="mt-2 text-sm text-slate-400">Sem evidências vinculadas</p>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-4">
          <SectionLabel className="text-red-700">Consequências potenciais de negócio</SectionLabel>
          <ul className="mt-2 space-y-1.5">
        ...
        {/* Responsible Area footer */}
        {responsibleArea && (
          <>
            <Separator className="bg-slate-100" />
            <div className="flex items-center gap-1.5 text-sm text-slate-500">
              <Building size={13} className="text-slate-400" strokeWidth={1.5} />
              Área responsável: 
              <span className="font-semibold text-slate-700">{responsibleArea}</span>
            </div>
          </>
        )}
      </div>
...
function SectionLabel({
  ...
  return (
    <span className={`text-xs font-bold uppercase tracking-widest ${className}`}>
      {children}
    </span>
  );
}

function RelPill({ label, isStart }: { label: string; isStart?: boolean }): ReactElement {
  return (
    <span
