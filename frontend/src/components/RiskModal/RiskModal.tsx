...
function validateRequiredText(value: string, message: string): string | undefined {
	if (!value.trim()) {
		return message;
	}
...
export default function RiskModal({
	...
	const [touchedFields, setTouchedFields] = useState<Record<RiskModalField, boolean>>({
		name: false,
		description: false,
		consequences: false,
	...
	const title = mode === "create" ? "Adicionar risco" : "Visualizar e editar risco";
	const subtitle =
		mode === "create"
				...
				<div className="grid gap-6 py-2">
					<section className="grid gap-4 rounded-xl border border-border/60 bg-muted/20 p-4">
						<div className="grid gap-2">
							...
							<Input
								...
								aria-describedby={showError("name") ? `${makeFieldId("name")}-error` : undefined}
							/>
							<FieldError id={`${makeFieldId("name")}-error`} message={showError("name")} />
						</div>

						<div className="grid gap-2">
							<Label htmlFor={makeFieldId("description")}>Descrição do risco</Label>
							<Textarea
								...
								aria-describedby={
									showError("description") ? `${makeFieldId("description")}-error` : undefined
								}
							/>
							<FieldError
						...
						<div className="grid gap-2">
							...
							<Textarea
								...
								onChange={(event) => handleFieldChange("consequences", event.target.value)}
								onBlur={() => handleBlur("consequences")}
								disabled={loading}
								className="min-h-[110px]"
								aria-invalid={Boolean(showError("consequences"))}
								aria-describedby={
									showError("consequences") ? `${makeFieldId("consequences")}-error` : undefined
								}
							/>
							<FieldError
					...
					<section className="grid gap-4 rounded-xl border border-border/60 bg-muted/20 p-4">
						...
						<div className="grid gap-4 sm:grid-cols-2">
							<div className="grid gap-2">
								<Label htmlFor={makeFieldId("occurrenceProbability")}>Probabilidade de ocorrência</Label>
								<Input
									...
									onChange={(event) =>
										handleFieldChange("occurrenceProbability", event.target.value)
									}
							...
							<div className="grid gap-2">
								<Label htmlFor={makeFieldId("impactProbability")}>Probabilidade de impacto</Label>
								<Input
									...
									onChange={(event) => handleFieldChange("impactProbability", event.target.value)}
						...
						<p className="text-xs text-muted-foreground">
							Use valores compatíveis com a configuração do projeto para manter a classificação
							consistente no cálculo do risco.
						</p>
					</section>
					...
					<section className="grid gap-4 rounded-xl border border-border/60 bg-muted/20 p-4">
						...
						<div className="grid gap-4">
							<div className="grid gap-2">
								<Label htmlFor={makeFieldId("damageOperations")}>Possíveis danos às operações</Label>
								<Textarea
									id={makeFieldId("damageOperations")}
									placeholder="Explique o impacto nas operações do cliente"
									value={form.damageOperations}
									onChange={(event) => handleFieldChange("damageOperations", event.target.value)}
							...
							<div className="grid gap-2">
								<Label htmlFor={makeFieldId("damageIndividuals")}>Possíveis danos aos indivíduos</Label>
								<Textarea
									id={makeFieldId("damageIndividuals")}
									placeholder="Explique o impacto sobre pessoas envolvidas"
									value={form.damageIndividuals}
									onChange={(event) => handleFieldChange("damageIndividuals", event.target.value)}
							...
							<div className="grid gap-2">
								<Label htmlFor={makeFieldId("damageOtherOrgs")}>Possíveis danos a outras organizações</Label>
								<Textarea
									id={makeFieldId("damageOtherOrgs")}
									placeholder="Explique impactos em parceiros ou terceiros"
									value={form.damageOtherOrgs}
									onChange={(event) => handleFieldChange("damageOtherOrgs", event.target.value)}
							...
							<div className="grid gap-2">
								<Label htmlFor={makeFieldId("recommendation")}>Recomendação</Label>
								<Textarea
									id={makeFieldId("recommendation")}
									placeholder="Plano de ação sugerido"
									value={form.recommendation}
									onChange={(event) => handleFieldChange("recommendation", event.target.value)}
					...
					<section className="grid gap-4 rounded-xl border border-border/60 bg-muted/20 p-4">
						...
						<div className="grid gap-2">
							...
							<p className="text-xs text-muted-foreground">
								Esse valor é normalizado e recalculado pelo sistema com base nas configurações do
								projeto.
							</p>
						</div>
					</section>
					...
					<div className="grid gap-4 lg:grid-cols-2">
						<LinkedOptionsSection
							...
							options={findings}
							selectedIds={findIds}
							onToggle={(id) => setFindIds((current) => toggleSelection(current, id))}
						...
						<LinkedOptionsSection
							...
							onToggle={(id) => setDamageAssetIds((current) => toggleSelection(current, id))}
